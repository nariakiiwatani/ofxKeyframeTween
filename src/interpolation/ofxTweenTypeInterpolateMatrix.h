...
namespace ofx { namespace tweentype { namespace interpolate {
	...
	struct Matrix4x4TRS {
		...
		static inline ofMatrix4x4 getInterpolated(float k, float k0, float k1, const ofMatrix4x4 &v0, const ofMatrix4x4 &v1, std::vector<ofEaseFunction> ease) {
			...
			t = Numeric<ofVec3f>::getInterpolated(k, k0, k1, t0, t1, utils::makeVectorFor<Numeric<ofVec3f>>(ease[0]));
			r = QuaternionSpherical::getInterpolated(k, k0, k1, r0, r1, utils::makeVectorFor<QuaternionSpherical>(ease[1]));
			s = Numeric<ofVec3f>::getInterpolated(k, k0, k1, s0, s1, utils::makeVectorFor<Numeric<ofVec3f>>(ease[2]));
			ofMatrix4x4 ret;
